...
The [.path]_404.hbs_ layout is similar to other layouts, except Antora automatically selects it create the 404 page.
With the exception of `page.layout` and `page.title`, the 404 page doesn't have access to the `page` variable in the UI model (as it's not applicable).
...
To use a page role, the AsciiDoc file for the page must declare the `page-role` attribute in the AsciiDoc document header.
